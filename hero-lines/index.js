...
import { Line, sliceBezier } from "./helpers.js";
import { simpleDebounce } from "../vendor/utilities.js";
...
class HeroLines {
  ...
  init() {
    ...
    window.addEventListener(
      "resize",
      simpleDebounce(this.onUpdateViewport, 200)
    );
    window.addEventListener(
      "scroll",
      simpleDebounce(this.onUpdateViewport, 200)
  ...
  createLines() {
    ...
    for (let i = 0; i < LINE_COUNT; i++) {
      ...
      for (let j = 0; j < PIVOT_COUNT; j++) {
        ...
        this.attractions[i][j] = new Physics.Attraction(
          this.big,
          this.pivots[i][j],
          150000,
          20.0
        ...
        this.physics.makeSpring(
          this.pivots[i][j],
          this.pivotsFixed[i][j],
          0.2,
          0.3,
          5
        );
  ...
  onUpdateViewport() {
    ...
    if (
      rect.width * this.pixelRatio !== this.root.width ||
  ...
  update({ line1, line2 }) {
    ...
    for (let j = 0; j < PIVOT_COUNT; j++) {
      ...
      this.pivotsFixed[LINE_COUNT - 1][j].position.set(
        newPosLast.x,
        newPosLast.y,
        0
      );
      if (!this.initiated) {
        ...
        this.pivots[LINE_COUNT - 1][j].position.set(
          newPosLast.x,
          newPosLast.y,
          0
        );
      ...
      for (let i = 1; i < LINE_COUNT - 1; i++) {
        const newX =
          newPosFirst.x + ((newPosLast.x - newPosFirst.x) * i) / LINE_COUNT;
        const newY =
          newPosFirst.y + ((newPosLast.y - newPosFirst.y) * i) / LINE_COUNT;
  ...
  draw() {
    const { context, pixelRatio, mode, root: canvas } = this;
    const { isInverted } = this.config;
    ...
    for (let i = 1; i < LINE_COUNT - 1; i++) {
      ...
      context.beginPath();
      context.moveTo(
        this.lines[i].p1.x * pixelRatio,
        this.lines[i].p1.y * pixelRatio
      );
      for (let j = 0, pivot; (pivot = line[j]); j++) {
        context.lineTo(
          pivot.position.x * pixelRatio,
          pivot.position.y * pixelRatio
        );
      }
      context.lineTo(
        this.lines[i].p2.x * pixelRatio,
        this.lines[i].p2.y * pixelRatio
      );
      context.stroke();
    }
  }
